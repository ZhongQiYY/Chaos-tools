            ...
            <div class="col-sm-2">
                <div class="box">
                    <div class="box-header with-border">
                        <h3 class="box-title">${genTable.comment!}树</h3>
                        <div class="box-tools">
...
<script type="text/javascript">
        ...
        krt.ajax({
            ...
            success: function (rb) {
                if (rb.code == 200) {
                    ...
                    $("#tree-table").remove();
                    var html = '<table id="tree-table" class="table table-bordered table-hover table-jk"><thead><tr><#list genTable.genTableColumns as column><#if column.isList=='0'><th>${column.comment}</th></#if></#list><th>操作</th></tr></thead><tbody id="tbody">' + tbody + '</tbody></table>';
    ...
    $("#insertBtn").click(function () {
        var pid = $("#pid").val();
        <#if formType=='tab'>
        top.krt.tab.openTab("新增${genTable.comment}","${"$"}{basePath}/${namespace}/${genTable.className?uncap_first}/insert?pid="+pid);
        </#if>
        <#if formType=='dialog'>
        top.krt.layer.openDialog("新增${genTable.comment}","${"$"}{basePath}/${namespace}/${genTable.className?uncap_first}/insert?pid="+pid,"1000px","500px");
        </#if>
    });
