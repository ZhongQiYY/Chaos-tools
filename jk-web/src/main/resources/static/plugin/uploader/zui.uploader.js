...
(function ($, window, Plupload, Moxie, undefined) {
	...
	var NAME = 'zui.uploader',
		FILE_TEMPLATE =
		'<div class="file"><div class="file-progress-bar"></div><div class="file-wrapper"><div class="file-icon"><i class="icon icon-file-o"></i></div><div class="jk-up-content"><div class="file-name"></div><div class="file-size small text-muted">0KB</div></div><div class="actions"><div class="file-status"  ><i class="icon"></i> <span class="text"></span></div><a   class="btn btn-link btn-download-file" target="_blank"><i class="icon icon-download-alt"></i></a><button type="button"   class="btn btn-link btn-reset-file" title="Repeat"><i class="icon icon-repeat"></i></button><button type="button"   class="btn btn-link btn-rename-file" title="Rename"><i class="icon icon-pencil"></i></button><button type="button"   title="Remove" class="btn btn-link btn-delete-file"><i class="icon icon-trash text-danger"></i></button></div></div></div>',
		DEFAULTS = {
			rename: true,
			renameByClick: true,
	...
	var Uploader = function (element, options) {
		...
		var lang = $.isPlainObject(options.lang) ? ($.extend(true, {}, Uploader.LANG[lang.lang || $.zui.clientLang()],
			options.lang)) : Uploader.LANG[options.lang];
		that.lang = lang;
		var $this = that.$;
		...
		if (options.browseByClickList || $list.hasClass('uploader-btn-browse')) {
			$list.addClass('uploader-btn-browse').on('click', '.file-wrapper > .actions,.file-renaming .file-name', function (
				e) {
				e.stopPropagation()
			})
		}
		...
		var $dropElement = (dropElement == 'fileList' ? that.$list : (dropElement == 'self' ? that.$ : $(dropElement))).first()
			.addClass('file-drag-area');
		if (!notSupportDnd) {
			var dropPlaceholder = options.dropPlaceholder;
			if (dropPlaceholder === true) dropPlaceholder = lang.dropPlaceholder;
			if (dropPlaceholder) $dropElement.find('.file-list').attr('data-drop-placeholder', dropPlaceholder)
		} else {
		...
		$list.on('click.' + NAME, '.btn-delete-file', function () {
			...
			if (file.status === Plupload.QUEUED || file.status === Plupload.FAILED || doneActionAble) {
				...
				if (deleteConfirmOption) {
					var confirmMessage = $.isFunction(deleteConfirmOption) ? deleteConfirmOption(file) : (deleteConfirmOption ===
						true ? lang.deleteConfirm : deleteConfirmOption);
		...
		if (options.rename) {
			$list.toggleClass('file-rename-by-click', !!options.renameByClick).toggleClass('file-show-rename-action-on-done', !
				!options.renameActionOnDone);
					...
					$filename.attr('contenteditable', 'true').one('blur', function () {
						...
						var renameFile = function () {
							if (filename !== undefined && filename !== null && filename !== '') {
								var ext = file.ext;
								if (ext.length && !options.renameExtension && filename.lastIndexOf('.' + ext) !== (filename.length - ext
										.length - 1)) {
									filename += '.' + ext
								}
								file.name = filename
	...
	Uploader.prototype.showMessage = function (message, type, time) {
		...
		var $content = $msg.find('.jk-up-content');
	...
	Uploader.prototype.previewImageSrc = function (file, callback) {
		...
		if (file.type == 'image/gif') {
			...
			console.log(fr.readAsDataURL(file.getSource()))
		} else {
			var preloader = new Moxie.image.Image();
			var resolveUrl = Moxie.core.utils.Url.resolveUrl;
			var ua = navigator.userAgent;
			if (ua.indexOf("compatible") > -1 && ua.indexOf("MSIE 9.0") > -1) {
				preloader.onload = function () {
					var thumb = $('#file-' + file.id + ' .file-icon');
					var optionSet = that.plupload.getOption();
					var ie9Size = optionSet.ie9Size;
					this.embed(thumb[0], {
						width: ie9Size.width,
						height: ie9Size.height,
						crop: true,
						fit: true,
						preserveHeaders: false,
						swf_url: resolveUrl(optionSet.flash_swf_url),
						xap_url: resolveUrl(optionSet.silverlight_xap_url)
					});
					preloader.destroy();
					preloader = null
				};
	...
	Uploader.prototype.createFileIcon = function (file) {
		...
		var type = (types && types.length) ? types[0] : '',
			subType = (types && types.length) > 1 ? types[1] : '';
		...
		else if (ext == 'js' || ext == 'php' || ext == 'cs' || ext == 'jsx' || ext == 'css' || ext == 'less' || ext ==
			'json' || ext == 'java' || ext == 'lua' || ext == 'py' || ext == 'c' || ext == 'cpp' || ext == 'swift' || ext ==
			'h' || ext == 'sh' || ext == 'rb' || ext == 'yml' || ext == 'ini' || ext == 'sql' || ext == 'xml') icon =
			'file-code';
		...
		return '<i class="icon icon-' + icon + ' file-icon-' + ext + '" data-type="' + fileType + '"' + (ext ?
			' data-ext="' + ext + '"' : '') + '></i>'
	};
	Uploader.prototype.getFileItem = function (file) {
		...
		if (!$file.length) {
			...
			$file.data('id', file.id).toggleClass('file-static', !!file.static).attr('id', 'file-' + file.id).appendTo(that.$list);
		}
		return $file
	};
	Uploader.prototype.showFile = function (file, responseObject) {
		...
		if (options.fileFormater) {
			...
			$file.find('.file-size').text((status == 'uploading' ? (Plupload.formatSize(Math.floor(file.size * file.percent /
				100)).toUpperCase() + '/') : '') + Plupload.formatSize(file.size).toUpperCase());
			$file.find('.file-icon').html(options.fileIconCreator ? options.fileIconCreator(file.type, file, that) : that.createFileIcon(
				file)).css('color', 'hsl(' + $.zui.strCode(file.type || file.ext) + ', 70%, 40%)');
		...
		if (options.previewImageIcon && file.isImage) {
			var setPreviewImage = function () {
				$file.find('.file-icon').html('<div class="file-icon-image" style="background-image: url(' + file.previewImage +
					')"></div>')
			};
	...
	Uploader.prototype.initPlupload = function () {
		...
		var plOptions = $.extend({}, options, {
			browse_button: that.$[0].id + "_browse",
			container: that.$[0],
		...
		var eventHandlers = {
			FilesAdded: function (uploader, files) {
				...
				if (limitFilesCount) {
					...
					if ((existCount + files.length) > limitFilesCount) {
						that.showMessage(that.lang.limitFilesCountMessage.format({
							count: limitFilesCount
						}), 'warning');
						var newFiles = [];
			...
			FileUploaded: function (uploader, file, responseObject) {
				...
				if (responseHandlerOption) {
					...
					if (error) {
						error = $.isPlainObject(error) ? error : {
							message: error
						};
			...
			UploadComplete: function (uploader, files) {
				...
				if (uploadedMessage) {
					...
					if (typeof uploadedMessage === 'string') {
						...
						msg += that.lang[failedCount > 0 ? 'uploadHasFailedMessage' : (uploadedCount > 0 ? 'uploadSuccessMessage' :
							'uploadEmptyMessage')].format(msgData)
					}
			...
			BeforeUpload: function (uploader, file) {
				...
				params = $.extend(params, $.isFunction(multipartParamsOption) ? multipartParamsOption(file, params) :
					multipartParamsOption);
			...
			Error: function (uploader, error) {
				var type = 'danger';
				if (error.code === Plupload.FILE_SIZE_ERROR || error.code === Plupload.FILE_SIZE_ERROR || error.code ===
					Plupload.FILE_EXTENSION_ERROR || error.code === Plupload.FILE_DUPLICATE_ERROR || error.code === Plupload.MAGE_FORMAT_ERROR
				) type = 'warning';
...
(function ($, grobal) {
	$.fn.krtUploader = function (options) {
		...
		var defaults = {
			...
			responseHandler: function (responseObject, file) {
				...
				var thisOptions = $(_this).data('zui.uploader').plupload.getOption();
				if (res.code == '200') {
					// 添加下载按钮
					
					if (thisOptions.resultCallBack != undefined && typeof thisOptions.resultCallBack == 'function') {
						thisOptions.resultCallBack(res.data)
					}
				} else {
					return '服务器返回错误：' + res.code + '  ：' + res.msg
				}
			},
			onUploadComplete: function (files) {
				var plupload = $(_this).data('zui.uploader');
				var thisOptions = plupload.plupload.getOption();
				var files = plupload.plupload.files;
				// btn-download-file增加下载按钮
				for(var l = 0; l < files.length; l++){
					if (files[l].remoteData) {
						if (files[l].remoteData.code == 200 && files[l].remoteData.data) {
							$(_this).find('#file-' + files[l].id).each(function () {
								if (files[l].remoteData.data) {
									$(this).find('.btn-download-file').prop('href',files[l].remoteData.data);
				...
				if (thisOptions.resultAllCallBack != undefined && typeof thisOptions.resultAllCallBack == 'function') {
					options.resultAllCallBack($(_this).getImgData())
				};
				var ua = navigator.userAgent;
				if (ua.indexOf("compatible") > -1 && ua.indexOf("MSIE 9.0") > -1) {
					
					for (var f = 0; f < files.length; f++) {
						if (files[f].remoteData) {
							if (files[f].remoteData.code == 200 && files[f].remoteData.data) {
								$(_this).find('#file-' + files[f].id).each(function () {
									var imgTypeArr = ['jpg', 'jpeg', 'bmp', 'gif', 'png'];
									var imgType = '';
									if (files[f].remoteData.data) {
										imgType = files[f].remoteData.data.split('.').pop();
										if (imgTypeArr.indexOf(imgType.toLowerCase()) >= 0) {
											$(this).find('.actions').siblings('.file-icon').html(
												'<div class="file-icon-image" style="background-image: url(' + files[f].remoteData.data +
												')"></div>')
										}
			...
			onFilesRemoved: function (files) {
				var uploader = $(_this).data('zui.uploader');
				var thisOptions = $(_this).data('zui.uploader').plupload.getOption();
				if (thisOptions.resultAllCallBack != undefined && typeof thisOptions.resultAllCallBack == 'function') {
					options.resultAllCallBack($(_this).getImgData())
				}
			...
			onInit: function () {
				if (options.btnDisabled) {
					$(_this).find('.uploader-btn-browse,.uploader-btn-start').prop('disabled', true).addClass('disabled');
					$(_this).find('.btn-delete-file').hide();
					$(_this).addClass('moxie-shim-hide')
				}
			}
		};
		if (options.backFiles) {
			var staticFiles = [];
			var backfiles = options.backFiles.split(',');
			for (var b = 0; b < backfiles.length; b++) {
				var imgsStr = backfiles[b].split('/');
				var name = '';
				var url = '';
				if (imgsStr.length > 1) {
					url = imgsStr.join('/');
					name = imgsStr.splice(imgsStr.length - 1, 1)
				}
				var item = {
					name: name[0],
					url: url
		...
		if (!options.jsPath) {
			...
			$('script').each(function () {
				var all = '';
				if ($(this).attr('src') && $(this).attr('src').indexOf('zui.uploader') >= 0) {
					all = $(this).attr('src').split('/');
		...
		var ua = navigator.userAgent;
		var upbtn = "";
		var btnSet = options.btn_set ? options.btn_set : '';
		var browseBtnClass = btnSet.browse_btn_class ? btnSet.browse_btn_class : '';
		var browseBtnIcon = btnSet.browse_btn_icon ? btnSet.browse_btn_icon : '';
		var startBtnClass = btnSet.start_btn_class ? btnSet.start_btn_class : '';
		var startBtnIcon = btnSet.start_btn_icon ? btnSet.start_btn_icon : '';
		var browseBtnText = btnSet.browse_btn_text ? btnSet.browse_btn_text : '';
		var startBtnText = btnSet.start_btn_text ? btnSet.start_btn_text : '';
		if (ua.indexOf("compatible") > -1 && ua.indexOf("MSIE 9.0") > -1) {
			upbtn = '<button type="button" class="btn ' + startBtnClass ? startBtnClass : 'btn-primary' +
				' uploader-btn-start" ><i class="'+(startBtnIcon?startBtnIcon:'icon icon-cloud-upload')+'"></i>' + startBtnText ? startBtnText : '开始上传' +
				'</button>'
		};
		var id = this[0].id;
		var htmls = {
			'photo': '<div class="photo-box"><div class="img-photo"><img src="' + options.jsPath +
				'/default_photo.png" alt=""></div><button type="button" class="btn ' + (browseBtnClass ? browseBtnClass :
					'btn-primary') + ' uploader-btn-browse" id="' + id + '_browse"><i class="'+(browseBtnIcon?browseBtnIcon:'icon icon-cloud-upload')+'" ></i> ' + (
					browseBtnText ? browseBtnText : '修改头像') +
				'</button></div><div class="file-list"  style="display: none;"></div>',
			'single': '<button type="button" class="btn ' + (browseBtnClass ? browseBtnClass : 'btn-primary') +
				' uploader-btn-browse"  id="' + id + '_browse"><i class="'+(browseBtnIcon?browseBtnIcon:'icon icon-cloud-upload')+'"></i> ' + (browseBtnText ?
					browseBtnText : '导入') + '</button><div class="file-list" style="display:none" ></div>',
			'simple': '<div class="uploader-message text-center"><div class="jk-up-content"></div><button type="button" class="close">×</button></div><div class="file-list" data-drag-placeholder="请拖拽文件到此处" data-drop-placeholder="请拖拽文件到此处"></div><button type="button" class="btn ' +
				(browseBtnClass ? browseBtnClass : 'btn-primary') + ' uploader-btn-browse"  id="' + id +
				'_browse"><i class="'+(browseBtnIcon?browseBtnIcon:'icon icon-cloud-upload')+'"></i> ' + (browseBtnText ? browseBtnText : '选择文件') + '</button> ' +
				upbtn,
			'lg-list': '<div class="uploader-message text-center"><div class="jk-up-content"></div><button type="button" class="close">×</button></div><div class="uploader-files file-list file-list-lg" data-drag-placeholder="请拖拽文件到此处"></div><div class="uploader-actions"><div class="uploader-status pull-right text-muted"></div><button type="button" class="btn ' +
				(browseBtnClass ? browseBtnClass : 'btn-link') + ' uploader-btn-browse"  id="' + id +
				'_browse"><i class="'+(browseBtnIcon?browseBtnIcon:'icon icon-plus')+'"></i> ' + (browseBtnText ? browseBtnText : '选择文件') +
				'</button><button type="button" class="btn ' + (startBtnClass ? startBtnClass : 'btn-link') +
				' uploader-btn-start"><i class="'+(startBtnIcon?startBtnIcon:'icon icon-cloud-upload')+'"></i> ' + (startBtnText ? startBtnText : '开始上传') +
				'</button></div>',
			'grid-list': '<div class="uploader-message text-center"><div class="jk-up-content"></div><button type="button" class="close">×</button></div><div class="uploader-files file-list file-list-grid"  data-drag-placeholder="请拖拽文件到此处"></div><div class="uploader-actions"><div class="uploader-status pull-right text-muted"></div><button type="button" class="btn ' +
				(browseBtnClass ? browseBtnClass : 'btn-link') + ' uploader-btn-browse"  id="' + id +
				'_browse"><i class="'+(browseBtnIcon?browseBtnIcon:'icon icon-plus')+'"></i> ' + (browseBtnText ? browseBtnText : '选择文件') +
				'</button><button type="button" class="btn ' + (startBtnClass ? startBtnClass : 'btn-link') +
				' uploader-btn-start"><i class="'+(startBtnIcon?startBtnIcon:'icon icon-cloud-upload')+'"></i> ' + (startBtnText ? startBtnText : '开始上传') +
		...
		if (options.type == 'simple' || options.type == 'single') {
			options.autoUpload = true
		};
		if (options.type == 'single') {
			options.fileTemplate = '<em style=display:none""></em>';
			options.runtimes = 'html5,silverlight,html4'
		};
		var photoSet = {
			filters: {
				mime_types: [{
		...
		if (options.type == 'photo') {
			options.runtimes = 'html5,silverlight,html4';
			options = $.extend({}, options, photoSet)
		};
		$(this).html(htmls[options.type]);
		options.flash_swf_url = options.jsPath + '/Moxie.swf';
		options.silverlight_xap_url = options.jsPath + '/Moxie.xap';
		var ie9Size = {
			single: {
				width: 0,
				height: 0
		...
		options.ie9Size = ie9Size[options.type];
		if (!options.multipart_params) {
			options.multipart_params = {}
		}
		if (!options.multipart_params.chunk_size) {
			options.multipart_params.chunk_size = options.chunk_size ? options.chunk_size : '1mb'
		};
		...
		var uploader=$(this).uploader(options);
		if (defaults.showDown != undefined && !defaults.showDown) {
			$(this).find('.btn-download-file').hide()
		};
		if (options.type == 'simple') {
			$(this).find(".btn-primary.uploader-btn-start").on('click', function () {
				var plupload = $(this).parents('.uploader').data('zui.uploader').plupload;
				plupload.start()
			})
		};
		var _this = this;
		var imgShow = setTimeout(function () {
			$(_this).find('.file-static').each(function () {
				$(this).find('.btn-download-file').each(function () {
					var imgSrc = $(this).attr('href');
					var imgType = '';
					var imgTypeArr = ['jpg', 'jpeg', 'bmp', 'gif', 'png'];
					if (imgSrc) {
						imgType = imgSrc.split('.').pop();
						if (imgTypeArr.indexOf(imgType.toLowerCase()) >= 0) {
							$(this).parent('.actions').siblings('.file-icon').append(
								'<div class="file-icon-image" style="background-image: url(' + imgSrc + ')"></div>')
						}
					}
				})
			});
			clearTimeout(imgShow)
		}, 50);
		return $(this).data('zui.uploader');;
	};
	$.fn.getImgData = function () {
		var imgStr = '';
		var imgArr = [];
		var staticFiles = [];
		if ($(this).krtUploader) {
			var plupload = $(this).data('zui.uploader');
			var files = plupload.plupload.files;
			$(this).find('.file-static a.btn-download-file').each(function () {
				staticFiles.push($(this).attr('href'))
			});
			for (var f = 0; f < files.length; f++) {
				if (files[f].remoteData) {
					if (files[f].remoteData.code == 200 && files[f].remoteData.data) {
						imgArr.push(files[f].remoteData.data)
					}
				}
			}
		};
		imgArr = staticFiles.concat(imgArr);
		console.log(imgArr);
		if (imgArr.length > 0) {
			imgStr = imgArr.join(',')
		};
	...
	$.fn.getPhotoData = function () {
		var img = $(this).getImgData();
		var photo = '';
		if (img) {
			photo = img.split(',').pop()
		}
